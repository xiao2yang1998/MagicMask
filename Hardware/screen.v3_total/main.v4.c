...
//        }
    ...
    //    printf("Pressure:%u\nTemperature:%u\nHumidity:%u\n",comp_data.pressure,comp_data.temperature,comp_data.humidity);
		...
		static char tempHum_key[2][18] = {"Temp","Rela"};
		static float tempHum_value[2];
        tempHum_value[0] = 0.77;
        tempHum_value[1] = 0.88;
        for(int i=0;i<2;i++){
						printf("===============================================\n");
            length = 0;
...
//        AT+ILOPSENDJSON=property,11
//{"co2":99}
        
        sprintf(co2_str,"%u",99);
		sprintf(total_str , "{\"co2\":");
        //strcat(total_str,"");
        strcat(total_str,co2_str);
        //strcat(total_str,"");
        strcat(total_str,"}");
        sprintf(tmp_str,"%d",10);
